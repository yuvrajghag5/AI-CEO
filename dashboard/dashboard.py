...
# ---------------------------------------------------------------- #
# Seeker fallback: when the model called a seeker tool but did NOT
# generate a full briefing, the panels still populate -- from that
# seeker's evidence. The seeker returns a formatted string; we parse it
# back into structured items so the panels render the same card style.
# ---------------------------------------------------------------- #
import re

_SEEKER_LINE = re.compile(
    r"\[([^\]]+)\]\s*(.+?)\s*—\s*(.*?)(?:\s*\(url:\s*(.+?)\))?$"
)


def parse_seeker_result(text):
    """Parse a seeker tool's formatted string into {confidence, impact, items}.

    Format produced by agent/tools.py `_format`:
        CATEGORY for 'topic' (confidence: 0.95, impact: High)
          [source] title — excerpt (url: ...)
          ...
    Returns None if not parseable; an empty items list if the seeker found
    no evidence.
    """
    if not text or not isinstance(text, str):
        return None
    if "No relevant" in text and "evidence found" in text:
        return {"confidence": None, "impact": None, "items": []}

    lines = text.split("\n")
    header = lines[0] if lines else ""
    conf = re.search(r"confidence:\s*([\d.]+)", header)
    imp = re.search(r"impact:\s*(\w+)", header)
    ...
    return {
        ...
        "impact": imp.group(1) if imp else None,
        "items": items,
    }


def get_seeker_data(result, tool_name):
    """Find a tool's result in result['evidence'] and parse it. None if the
    tool wasn't called this turn."""
    for ev in (result or {}).get("evidence", []):
        if ev.get("tool") == tool_name:
            return parse_seeker_result(ev.get("result", ""))
    return None


def render_seeker_cards(data, css_class=""):
    """Render parsed seeker items as evidence cards (the no-briefing view)."""
    if data and data.get("confidence") is not None:
        st.markdown(
            f'<p style="color:{TEXT_MUTED};font-size:0.85rem;">'
            f'Confidence: {data["confidence"]} · Impact: {data.get("impact","—")} '
            f'· from direct seeker (no full briefing this turn)</p>',
            unsafe_allow_html=True,
        )
    for it in (data or {}).get("items", []):
        title = it.get("title", "")
        if it.get("url") and it["url"] != "(no url available)":
            title = f'<a href="{it["url"]}" target="_blank">{title}</a>'
        st.markdown(
            f"""<div class="evidence-card {css_class}">
                    <span class="badge" style="background-color:{NVIDIA_GREEN}22;color:{NVIDIA_GREEN};">{it.get('source','')}</span>
                    <h4 style="margin-top:8px;">{title}</h4>
                    <p style="color:{TEXT_MUTED};font-size:0.85rem;">{it.get('excerpt','')}…</p>
                </div>""",
            unsafe_allow_html=True,
        )
...
def panel_market_intel(result):
    ...
    # no briefing -> fall back to the competitor and trend seekers
    comp = get_seeker_data(result, "competitor_activity_seeker")
    trend = get_seeker_data(result, "trend_seeker")
    shown = False
    if comp and comp.get("items"):
        st.markdown("#### Competitor Activity")
        render_seeker_cards(comp, css_class="competitor")
        shown = True
    if trend and trend.get("items"):
        st.markdown("#### Emerging Technologies & Trends")
        render_seeker_cards(trend, css_class="trend")
        shown = True
    if not shown:
        empty("Ask a strategic question to populate market intelligence.")
...
def panel_opportunities(result):
    ...
    if b:
        ...
        return
    # no briefing -> fall back to the opportunity_seeker's evidence
    data = get_seeker_data(result, "opportunity_seeker")
    if data and data.get("items"):
        render_seeker_cards(data)
    else:
        empty("No opportunities yet — ask the agent a question.")
...
def panel_risks(result):
    ...
    if b:
        ...
        return
    data = get_seeker_data(result, "risk_seeker")
    if data and data.get("items"):
        render_seeker_cards(data, css_class="risk")
    else:
        empty("No risks yet — ask the agent a question.")
...
def panel_sources(result):
    """
    Dedicated Sources panel: every source behind the latest answer. Prefers
    the briefing's _sources (with S-IDs); falls back to the sources from
    whatever seeker tools ran when there's no full briefing.
    """
    st.title("Sources Used")
    b = (result or {}).get("briefing")
    sources = (b or {}).get("_sources", {})
    ...
    # no briefing -> gather sources from whatever seekers ran, deduped
    seen = set()
    any_shown = False
    for tool in ("opportunity_seeker", "risk_seeker", "trend_seeker",
                 "competitor_activity_seeker"):
        data = get_seeker_data(result, tool)
        for it in (data or {}).get("items", []):
            ...
            if not any_shown:
                st.markdown(
                    f'<p style="color:{TEXT_MUTED};">Sources from the seeker(s) '
                    f'called this turn (no full briefing generated).</p>',
                    unsafe_allow_html=True,
                )
                any_shown = True
            ...
            color = src_colors.get(provider, TEXT_MUTED)
            url = it.get("url", "")
            title_html = it.get("title", "(untitled)")
            ...
            st.markdown(
                f"""<div class="evidence-card">
                        <span class="badge" style="background-color:{color}22;color:{color};">{provider}</span>
                        <h4 style="margin-top:8px;">{title_html}</h4>
                        <p style="color:{TEXT_MUTED};font-size:0.85rem;">{(it.get('excerpt','') or '')[:280]}…</p>
                    </div>""",
                unsafe_allow_html=True,
            )

    if not any_shown:
        empty("Sources appear here once you ask a question.")
